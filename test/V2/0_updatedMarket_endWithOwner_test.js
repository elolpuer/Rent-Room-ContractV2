...
describe("Updated (With custom token) Market End With Owner", function () {
  ...
  it("Should rent property", async () => {
    ...
    await rent.endRentOwnerWithCustomTokenPayment("1")
    ...
    await expect(
      rent.connect(renter).endRentRenterWithCustomTokenPayment("1")
    ).to.be.revertedWith("Marketplace: You not a renter")
  })
